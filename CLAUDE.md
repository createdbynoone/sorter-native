...
- **Inspector flotante** (`InspectorOverlay`, overlay sobre grid y focus, como Electron): tarjeta redondeada (16 pt, inset 12) con Liquid Glass nativo (`GlassSurface`: `glassEffect(.regular)` tintado `Theme.surface` 55 % — sin tinte las fotos lavan el texto, al 80 % parece opaco; fallback material en < macOS 26) + botón redondo de 40 pt centrado verticalmente que se desliza con el panel y rota el chevron. El contenido del inspector usa `secondary`/`text` al 85 % y bordes `hairlineStrong` porque va sobre superficie translúcida. Estado, nota (autosave 400 ms), categoría + producto (radio en cada nivel, `+` para crear, click derecho renombrar/borrar).
